...
+ (void)getVideoPathWithAsset:(PHAsset *)asset withOutputDir:(NSString *)outputDir completion:(void (^)(NSString * filePath, NSError * error))completion {
    ...
    [self getVideoWithAsset:asset progressHandler:^(double progress, NSError * _Nonnull error, BOOL * _Nonnull stop, NSDictionary * _Nonnull info) {
        ...
        if(([avAsset isKindOfClass:[AVComposition class]] && ((AVComposition *)avAsset).tracks.count == 2)){
            ...
            NSString *videoName = [urlAsset.URL lastPathComponent]; // 获取原始文件名
            if (videoName.length == 0) {
                videoName = [NSString stringWithFormat:@"%@.MOV", [NSUUID UUID].UUIDString];
            }
            NSString *filePath = [outputDir stringByAppendingPathComponent:videoName];
            ...
            if ([fileManager fileExistsAtPath:filePath]) {
                [fileManager removeItemAtPath:filePath error:nil]; // 如果文件已存在，先删除
            }
            
            NSError *error = nil;
            [fileManager copyItemAtURL:urlAsset.URL toURL:[NSURL URLWithString:[@"file://" stringByAppendingString:filePath]] error:&error];
            if (error) {
                NSLog(@"导出视频时发生错误: %@", error);
                dispatch_async(dispatch_get_main_queue(), ^{
                    completion(nil, error);
                });
            } else {
                NSLog(@"视频成功导出到：%@", filePath);
                dispatch_async(dispatch_get_main_queue(), ^{
                    completion(filePath, nil);
                });
            }
